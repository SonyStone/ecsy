...
describe('system', () => {
  ...
  it('Empty queries', () => {
    ...
    // System 1
    @SystemData()
    class SystemEmpty implements System {
      run() {}
    }

    // Register empty system
    world.registerSystem(SystemEmpty);

    expect(world.systemManager.getSystem(SystemEmpty).queries).toEqual([]);
  });
  ...
  it('Queries', () => {
    ...
    class SystemBar implements System {
      run() {}
    }

    @SystemData(
      [Read(FooComponent), Read(BarComponent)]
    )
    class SystemBoth implements System {
      run() {}
    }
    ...
    // Foo
    expect(world.systemManager.getSystem(SystemFoo).queries[0].components.length).toBe(10);
    // Bar
    expect(world.systemManager.getSystem(SystemBar).queries[0].components.length).toBe(10);
    // Both
    expect(world.systemManager.getSystem(SystemBoth).queries[0].components.length).toBe(5);
  });

  describe('Queries with \'Not\' operator', () => {
    let world: World;
    ...
    it('empty', () => {
      ...
      @SystemData(
        [Not(FooComponent), Not(BarComponent)]
      )
      class SystemNotNot implements System {
        run() {}
      }
      ...
      expect(world.systemManager.getSystem(SystemNotNot).queries[0].components.length).toEqual(0);
    })

    it('select', () => {

      @SystemData(
        [Read(FooComponent), Not(BarComponent)],
        [Read(EmptyComponent), Not(BarComponent)],
        [Read(EmptyComponent), Not(BarComponent), Not(FooComponent)],
      )
      class SystemNotBar implements System {
        run() {}
      }
      ...
      expect(queries[0].components.length).toBe(5);
      expect(queries[1].components.length).toBe(5);
      expect(queries[2].components.length).toBe(0);
      ...
      expect(queries[0].components.length).toBe(4);
      expect(queries[1].components.length).toBe(4);
